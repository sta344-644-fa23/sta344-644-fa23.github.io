...
X <- model.matrix(~scale(pcaff), data = lip_cancer)

d <- list(n = nrow(X),         # number of observations
             p = ncol(X),         # number of coefficients
             X = X,               # design matrix
             y = lip_cancer$Observed,               # observed number of cases
             log_offset = log(lip_cancer$Expected), # log(expected) num. cases
             W_n = sum(A) / 2,    # number of neighbor pairs
             W = A)               # adjacency matrix


s = rstan::sampling(car_m, data = d, cores = 4, iter=1e4)
