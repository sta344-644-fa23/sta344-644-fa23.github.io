...
```{r}
spdep::moran.test(
  lip_cancer$Observed / lip_cancer$Expected, listw
)
```
:::
::::
...
b_car = brms::brm(
  ...
  family = poisson, cores=4, iter=10000,
...
```{r echo=FALSE}
ggplot(bar_pred, aes(x=Observed, y=y_pred)) +
  geom_abline(intercept=0, slope=1, color="grey") +
  geom_point() +
